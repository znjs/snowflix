...
import { PlayListModalType } from "types/component-types";
import { PlaylistType } from "types/video-types";
import { addVideoToPlaylist, createNewPlaylist, removeVideoFromPlaylist } from "../../utils";
import "./playlistModal.css";

const PlayListModal: PlayListModalType = ({ playlistModalData, setPlaylistModalData }) => {
  ...
  }, [playlistModalData.showModal, newPlaylistState.createState, fetchPlaylist, encodedToken]);
  return (
    <div
      className={`${
      ...
      <div className="bg-clr-gray-900 clr-gray-50 pd-1 brd-sm w-min-16">
        <div className="fx fx-jc-sb fx-ai-center">
          ...
          <i
            ...
            onClick={() => {
              setPlaylistModalData((prev: PlayListModalType) => ({ ...prev, showModal: false }));
            }}></i>
        </div>
        <hr />
          ...
          {playlistData?.map((item, idx) => (
            <div key={item._id} className="fx fx-ai-center">
              <input
                ...
                id={`option ${idx}`}
                name={`option ${idx}`}
                checked={Boolean(item?.videos.find((singleVideo) => singleVideo._id === video._id))}
                onChange={() => {
